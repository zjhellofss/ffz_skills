#!/usr/bin/env python3
"""半导体产业链基本面评分器（本地结构化数据版，规则 v2）。

兼容模式可读取旧长表并复算诊断分；严格模式要求 v2 输入引用通过
本 skill 本地 CSV 溯源校验的事实快照，才允许发布正式/暂定评级和同业排名。

示例::

    python3 score.py /path/to/score-input.csv --mode strict \
      --facts /path/to/local-facts.csv \
      --source-root /home/fss/data/上市公司财务信息 \
      --out-dir /tmp/score-run
"""
...
def run_fact_validator(facts_path: Path, source_root: Path | None, skip_source_check: bool) -> dict[str, Any]:
    ...
    from validate_local_facts import validate_file
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--facts", type=Path, help="本地结构化数据事实快照；strict必需")
    parser.add_argument("--source-root", type=Path, help="/home/fss/data下数据集根目录")
    parser.add_argument("--skip-source-check", action="store_true", help="本地严格模式禁用")
