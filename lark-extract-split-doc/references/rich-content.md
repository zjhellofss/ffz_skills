...
Write a snapshot to `embedded/sheet-<sequence>-<sheet-id>.md`. Preserve formatted cell values, row order, and columns. Add a local link and source metadata at the original block location in `source.md`. Inline a readable Markdown or HTML table when reasonably sized. The local link and metadata are export scaffolding; remove them from numbered split files unless the user asks to retain per-segment provenance.
...
When writing `split/*.md`, keep only the human-meaningful inline table or preview. Drop the wrapper heading if it merely says `嵌入电子表格` plus a token/sheet ID, drop `[查看完整快照](...)`, and drop the adjacent `lark-resource` HTML comment. If the embedded sheet has a real user-authored caption or title, preserve that caption as normal content.
...
Write `embedded/base-<sequence>-<table-id>.md`. Keep a visible local link at the original location in `source.md` and inline a readable preview or full table when reasonably sized. Preserve table name, field names, row order, and pagination limitations in `manifest.md`.

When writing `split/*.md`, keep only the human-meaningful inline table or preview. Drop generic wrapper headings, complete-snapshot links, and adjacent `lark-resource` comments unless the user requested provenance inside split files.
...
Markdown has no portable transclusion mechanism. To keep data visible after splitting while keeping segments readable:

- Keep downloaded images embedded with adjusted local paths.
- Keep native tables inline.
- Keep a compact inline preview for large Sheet or Base snapshots.
- Drop export-only snapshot links, generic embedded-resource headings, and source metadata comments from numbered files by default.
